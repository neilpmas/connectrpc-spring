...
// don't use Spring Boot (see ConnectFilterEndToEndTests), so a plain AnnotationConfigApplicationContext
// stands in for the real ApplicationContext that getBeansWithAnnotation() needs.
class GlobalConnectInterceptorEndToEndTests {
	...
	void globalInterceptor_shouldRunBeforeServiceMethod() {
		try (var context = new AnnotationConfigApplicationContext(RejectingInterceptorConfig.class)) {
			ConnectServiceRegistry registry = new ConnectServiceRegistry(List.of(new GreetServiceImpl()), context);
			...
			webTestClient.post()
				...
				.jsonPath("$.message")
				.isEqualTo("rejected by test interceptor");
		}
	}
	...
	@Test
	@SuppressWarnings("unchecked") // getBean(name, Class) can't express List<String> due
									// to erasure;
	...
	private static WebTestClient webTestClient(ConnectServiceRegistry registry) {
		ConnectFilter filter = new ConnectFilter(registry, "/connect", DataSize.ofMegabytes(4).toBytes(), false,
				List.of());
