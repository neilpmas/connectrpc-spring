...
public class ConnectFilter implements WebFilter {
	...
	// Per the Connect protocol spec (https://connectrpc.com/docs/protocol#unary-request),
	// the only
	// defined protocol version is "1", and Timeout-Milliseconds is a positive integer as
	// an ASCII
	...
	// CORS preflight handling per https://connectrpc.com/docs/cors/. This filter only
	// serves the
	// unary POST variant of the Connect protocol (no gRPC-Web), so the allowed method
	// list and
	// headers are scoped accordingly; gRPC-Web trailer headers like Grpc-Status don't
	// apply here.
	private static final String CORS_ALLOWED_METHODS = "POST";

	private static final String CORS_ALLOWED_HEADERS = "Content-Type, Connect-Protocol-Version, Connect-Timeout-Ms, X-User-Agent";
	...
	// Mapping per the Connect protocol spec:
	...
	private static final Map<Status.Code, ConnectError> STATUS_MAP = Map.ofEntries(
			...
			Map.entry(Status.Code.RESOURCE_EXHAUSTED,
					new ConnectError("resource_exhausted", HttpStatus.TOO_MANY_REQUESTS)),
	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
		...
		if (this.corsEnabled && HttpMethod.OPTIONS.equals(request.getMethod()) && path.startsWith(this.pathPrefix)) {
			return handleCorsPreflight(exchange);
		}

		if (!HttpMethod.POST.equals(request.getMethod()) || !path.startsWith(this.pathPrefix)) {
		...
		String remaining = path.substring(this.pathPrefix.length());
		...
		ConnectServiceRegistry.MethodEntry entry = this.registry.lookup(serviceName, methodName);
		if (entry == null) {
		...
		// Unary content types are "application/" followed by the codec name ("proto" or
		// "json").
		...
		if (contentType != null && APPLICATION_PROTO.isCompatibleWith(contentType)) {
			...
			return writeConnectError(exchange.getResponse(), "unknown", "Unsupported Content-Type: " + contentType,
		...
		return DataBufferUtils.join(request.getBody())
			...
			.flatMap((dataBuffer) -> {
				...
				if (byteCount > this.maxMessageSizeBytes) {
					DataBufferUtils.release(dataBuffer);
					return writeConnectError(exchange.getResponse(), "resource_exhausted",
							"Message size " + byteCount + " exceeds maximum " + this.maxMessageSizeBytes + " bytes",
	...
	// CORS preflight response per https://connectrpc.com/docs/cors/: no gRPC method is
	// invoked,
	...
	// Resolves Access-Control-Allow-Origin against the configured allow-list and sets it
	// on the
	// response, if any origin is allowed. If the list contains "*", any origin is allowed
	// (echoing
	// the request's Origin header back, or "*" literally if there's no Origin header).
	// Otherwise,
	// only an exact match against the configured list is allowed. An unmatched origin
	// simply gets no
	// CORS header at all (not an error) -- the browser then blocks the response
	// client-side.
	private void applyCorsAllowOrigin(ServerHttpRequest request, ServerHttpResponse response) {
		...
		else if (origin != null && this.corsAllowedOrigins.contains(origin)) {
			response.getHeaders().add("Access-Control-Allow-Origin", origin);
		}
	}

	@SuppressWarnings("unchecked")
	private Mono<Void> invokeMethod(ServerWebExchange exchange, ConnectServiceRegistry.MethodEntry entry,
			byte[] bodyBytes, Codec codec, @Nullable Duration timeout) {

		MethodDescriptor<Object, Object> descriptor = (MethodDescriptor<Object, Object>) entry.descriptor();
		ServerCallHandler<Object, Object> handler = (ServerCallHandler<Object, Object>) entry.methodDefinition()
		...
		try {
			...
			return writeConnectError(exchange.getResponse(), "invalid_argument",
					"Failed to parse request: " + ex.getMessage(), HttpStatus.BAD_REQUEST);
		}
		...
		return ReactiveSecurityContextHolder.getContext()
			...
			.flatMap((securityContext) -> {
				...
				Mono<Object> invocation = Mono.fromCallable(() -> {
					...
					try {
						// Drives the real ServerCallHandler exactly as a real gRPC server
						// would for
						// a unary call: startCall() hands back a Listener, which we feed
						// the single
						// request message and then half-close, per the
						// ServerCall.Listener contract
						// ("onMessage, which is guaranteed before half close"). The
						// generated
						// ServerCalls.asyncUnaryCall handler invokes the actual service
						// method from
						// onHalfClose() and reports the result back via
						// call.sendMessage()/close(),
						// which SynthesizedServerCall translates into the future's
						// completion.
				...
				return invocation.flatMap((response) -> switch (codec) {
	...
	private Mono<Void> writeProtoResponse(ServerHttpResponse response, MethodDescriptor<Object, Object> descriptor,
			Object message) {
		response.setStatusCode(HttpStatus.OK);
		response.getHeaders().setContentType(APPLICATION_PROTO);
		// This runs on Schedulers.boundedElastic() (see the subscribeOn() upstream in
		// invokeMethod()),
		// which is designed for exactly this kind of bounded blocking work.
		// streamResponse() for
		// protobuf messages also just backs onto an in-memory ByteArrayInputStream, not
		// real I/O, so
		// readAllBytes() never actually blocks on anything. IntelliJ's inspection doesn't
		// trace
		...
		try (var stream = descriptor.streamResponse(message)) {
			...
			return writeConnectError(response, "internal", "Failed to serialize response",
	...
	@SuppressWarnings("DataFlowIssue") // PrototypeMarshaller contract guarantees a
										// non-null prototype
	private Object parseJsonRequest(MethodDescriptor<Object, Object> descriptor, byte[] bodyBytes) throws Exception {
		MethodDescriptor.PrototypeMarshaller<Object> marshaller = (MethodDescriptor.PrototypeMarshaller<Object>) descriptor
	...
	private Mono<Void> handleError(ServerHttpResponse response, Throwable throwable) {
		if (throwable instanceof TimeoutException) {
			// "Deadline expired before RPC could complete": deadline_exceeded maps to
			// HTTP 504 per
			// https://connectrpc.com/docs/protocol#error-codes
			return writeConnectError(response, "deadline_exceeded", "The operation timed out",
		...
		if (throwable instanceof StatusRuntimeException sre) {
			...
			// StatusProto.fromThrowable() decodes the binary grpc-status-details-bin
			// trailer (if the
			// service attached one via StatusProto.toStatusRuntimeException()) back into
			// a
	...
	private Mono<Void> writeConnectError(ServerHttpResponse response, String code, @Nullable String message,
			HttpStatusCode status) {
		return writeConnectError(response, code, message, status, List.of());
	}

	// Structured error details per https://connectrpc.com/docs/protocol#error-codes: each
	// detail's
	// "type" is the fully-qualified Protobuf message name (the Any type URL with the
	// "type.googleapis.com/" prefix stripped), and "value" is the unpadded, standard
	// base64 encoding
	// of the message's serialized bytes.
	private Mono<Void> writeConnectError(ServerHttpResponse response, String code, @Nullable String message,
			HttpStatusCode status, List<Any> details) {
