...
final class SynthesizedServerCall<ReqT, RespT> extends ServerCall<ReqT, RespT> {
    ...
    // Only unset if a misbehaving service calls close() without sendMessage() first; well-behaved
    ...
    // a well-behaved unary call always sent a message before closing OK, but this can't be proven statically.
    ...
    public boolean isCancelled() {
        // No real client connection, so this call is never cancelled out from under the handler.
        return false;
    }

    @Override
    public void request(int numMessages) {
        // No-op: flow control has no meaning for a synthetic, single-message in-process call.
    }
