...
class ConnectFilterEndToEndTests {
	...
	private static WebTestClient webTestClient() {
		...
		ConnectFilter filter = new ConnectFilter(registry, "/connect", DataSize.ofMegabytes(4).toBytes(), true,
				List.of("*"));
	...
	void sayHello_withJsonContentType_shouldReturnJsonResponse() throws Exception {
		String responseJson = this.webTestClient.post()
			.uri("/connect/greet.v1.GreetService/SayHello")
	...
	void unknownMethod_shouldReturn404WithUnimplementedCode() {
		this.webTestClient.post()
			...
			.expectBody()
			.jsonPath("$.code")
			.isEqualTo("unimplemented");
	}

	@Test
	void resourceExhausted_shouldReturn429WithResourceExhaustedCode() {
		SayHelloRequest request = SayHelloRequest.newBuilder().setName("trigger-resource-exhausted").build();
	...
	void cancelled_shouldReturn499WithCanceledCode() {
		SayHelloRequest request = SayHelloRequest.newBuilder().setName("trigger-cancelled").build();
	...
	void corsPreflight_shouldReturn204WithAllowHeaders() {
		this.webTestClient.options()
			...
			.expectHeader()
			.valueEquals("Access-Control-Allow-Headers",
					"Content-Type, Connect-Protocol-Version, Connect-Timeout-Ms, X-User-Agent")
			.expectHeader()
			.valueEquals("Access-Control-Max-Age", "7200")
			.expectHeader()
			.valueEquals("Vary", "Origin, Access-Control-Request-Method, Access-Control-Request-Headers");
	}
	...
	void triggerDetails_shouldReturnUnavailableWithStructuredDetails() throws Exception {
		SayHelloRequest request = SayHelloRequest.newBuilder().setName("trigger-details").build();
		...
		byte[] responseBytes = this.webTestClient.post()
			...
			.jsonPath("$.message")
			.isEqualTo("overloaded: back off and retry")
			.jsonPath("$.details[0].type")
